 ...
 * `state.theme`/the theme button/the config-panel color UIs/a fresh render
 * -- `state.theme` (`editor/js/state.ts`) and `config-panel.ts`/
 * `rendering.ts`/`theme-button.ts` all migrate to React in #808-#810, not
 * this issue, and `demo/components/*.tsx` doesn't import from
 * `editor/js/*.ts` regardless (see `demo/components/editor-app.tsx`'s
...
// by the time it runs, `config-panel.ts`, `rendering.ts`, and
// `theme-button.ts` (imported above) have already finished their own
// top-level DOM-ref setup, exactly as when this call lived further down in
// init.ts (see that file's own former comment: "must happen after all DOM
// refs + functions are ready").
