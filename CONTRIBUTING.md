...
- `scripts/docker-test-visual-ascii.sh` — run the ASCII half of the visual suite inside the font-corrected Linux container, against the `-chromium-linux.png` baselines CI gates on (needs Docker; ASCII only, SVG is not supported through this path — see "Visual regression tests" below)
...
Don't trust a **stock** `mcr.microsoft.com/playwright:*` Docker container as a stand-in for the real `ubuntu-latest` CI runner when checking whether a `-chromium-linux.png` baseline is stale — confirmed in [#326](https://github.com/dfadler/zombie-mermaid/issues/326) to render the ASCII/terminal-panel samples ~100px wider than CI actually does (a font-substitution difference between the image's own bundled fonts and what `playwright install --with-deps chromium` installs on bare `ubuntu-latest`), producing baseline "staleness" that doesn't reproduce in CI at all. If a linux baseline is suspected stale, verify against an actual CI run (or its `visual-regression-failures-<shard>` artifacts — the job is sharded 4-way, so a failure can land in any one of them) rather than a local Docker approximation.

#### Checking the Linux ASCII baselines locally (containerized)

That caveat has since been **narrowed for ASCII samples, and only for ASCII samples.**

[#614](https://github.com/dfadler/zombie-mermaid/issues/614) root-caused #326's width inflation and fixed it: the stock image ships no DejaVu family, so fontconfig resolves the generic CSS `monospace` family to `WenQuanYi Zen Hei Mono`, a CJK face that renders box-drawing glyphs at full width. (The named faces `.ascii-panel`'s CSS asks for are irrelevant here — the grid lives in a `<code>`, and Chromium's UA stylesheet resets that element to bare `monospace`, so only fontconfig's generic alias matters. Installing them makes things _worse_.) Adding `fonts-dejavu-core` — `docker/visual-regression.Dockerfile`, which also asserts the resolution at build time so a base-tag bump can't silently regress it — takes `ascii-samples.visual.test.ts` from 88/90 failing to **0/90** against the same committed, unmodified `-chromium-linux.png` baselines. [#615](https://github.com/dfadler/zombie-mermaid/issues/615) separately showed architecture contributes nothing to ASCII output (arm64 vs. emulated amd64: max per-pixel channel delta 1/255, against a `threshold: 0.4`), so an Apple Silicon machine can run this natively rather than under emulation and still get CI-accurate results.

So a macOS contributor _can_ now check the Linux ASCII baselines — the ones CI actually gates on, which a local `pnpm run test:visual` never touches because it compares against `-chromium-darwin.png`:

```bash
scripts/docker-test-visual-ascii.sh            # build the image if needed, then run
scripts/docker-test-visual-ascii.sh --help     # options, exit codes, and the full rationale
```

It syncs a copy of your working tree (uncommitted changes included) into a cache directory outside the repo, runs the container as your own UID/GID so nothing it writes is root-owned, and never mounts the repo writable — so it cannot modify a committed baseline. `--update-snapshots` is rejected outright: the committed PNGs are the measuring instrument for #614/#615/#549, and regenerating them from a container destroys that. Failure diffs and the HTML report land in the cache directory, whose path the script prints.

**The #326 caveat stands unchanged for SVG samples**, which is why the wrapper refuses any target other than the ASCII test file. #615 measured native arm64 producing ~5% intermittent SVG failures against the shared x86 baselines — different samples each run, not sub-pixel — while `--platform linux/amd64` avoids them entirely but costs ~11x wall clock (24.4m vs. 2.2m for the full suite). Neither is a workflow worth recommending until [#549](https://github.com/dfadler/zombie-mermaid/issues/549) runs the container in real CI and settles what the shared target is; [#551](https://github.com/dfadler/zombie-mermaid/issues/551) (baseline consolidation) is blocked on the same thing. A suspected-stale SVG `-chromium-linux.png` still has to be verified against a real CI run. The full reasoning, including what remains unmeasured, is in the 2026-09-07 amendment to [`docs/decisions/playwright-docker-image-visual-regression.md`](./docs/decisions/playwright-docker-image-visual-regression.md).
