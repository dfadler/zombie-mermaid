...
- `'static'` (default) — preserves today's output as closely as possible: edge animation still renders where a diagram opts into it; xychart hover tooltips stay off unless requested via `'full'` or the deprecated `interactive: true`.
- `'full'` — also enables xychart hover tooltips.

`click`-based links and `<title>` tooltips are not yet gated by this option in any mode.
