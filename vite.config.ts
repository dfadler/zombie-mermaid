...
async function rebuild(server: ViteDevServer): Promise<void> {
  ...
  const [samplesExit, editorExit, pagesExit] = await Promise.all([
  ...
  if (samplesExit === 0 && editorExit === 0 && pagesExit === 0) {
    console.log(`\x1b[32m[dev]\x1b[0m Rebuilt in ${ms}ms`)
    server.ws.send({ type: 'full-reload' })
  } else {
    console.error(
      `\x1b[31m[dev]\x1b[0m Build failed (samples exit ${samplesExit}, editor exit ${editorExit}, pages exit ${pagesExit})`,
    )
  }
