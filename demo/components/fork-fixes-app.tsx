...
/**
 * The "what this fork fixes" before/after showcase's *hydrated* content
 * (zombie-mermaid#802) — the hero header and the fixes list, everything
 * inside {@link FORK_FIXES_ROOT_ID}'s hydration boundary. Split out of
 * `fork-fixes-page.tsx` (which used to be a full-document owner, like
 ...
 * `dashboard-app.tsx`'s split, and for the identical reason: `demo/
 * fork-fixes-client.tsx` (the browser bundle `fork-fixes.ts` inlines)
 * imports {@link ForkFixesApp} from *this* file, not from
 ...
 * unchanged — this split moves *how* it's rendered (SSR + hydrate instead
 * of SSR-only), never *what*. `PanelContent`'s `ascii`/`svg` `html` strings
 * and `symptomHtml`/`lookForHtml` are already plain, pre-rendered strings
 * (from ascii-html.ts/formatProse), so they round-trip through
 * `JSON.stringify`/`JSON.parse` for hydration with no re-render of any
 * Node-only pipeline (shiki, the real-terminal capture pipeline) in the
 * browser — see fork-fixes.ts's own header for those guarantees.
 *
...
/**
 * `fork-fixes-root`: id of the *hydration container*
 * `demo/fork-fixes-client.tsx`'s `hydrateRoot()` call mounts onto
 * (zombie-mermaid#802) — a plain wrapper `<div>` `fork-fixes-page.tsx`'s
 * `ForkFixesPage` renders directly, not part of {@link ForkFixesApp}'s own
 * render output. See `dashboard-app.tsx`'s `DASHBOARD_ROOT_ID` doc comment
 * for why this has to be a separate element from `ForkFixesApp`'s own root
 * rather than an id on that component itself.
 */
export const FORK_FIXES_ROOT_ID = 'fork-fixes-root'

/**
 * `fork-fixes-props`: the `<script type="application/json">` element
 * `demo/fork-fixes-client.tsx` reads {@link ForkFixesAppProps} out of --
 * the exact same props object `fork-fixes-page.tsx` renders `ForkFixesApp`
 * with server-side, so hydration runs against identical data (no
 * `ascii-html.ts`/shiki/real-terminal re-render in the browser -- see this
 * file's header comment).
 */
export const FORK_FIXES_PROPS_ELEMENT_ID = 'fork-fixes-props'

export interface ForkFixesAppProps {
  fixes: readonly FixSectionProps[]
}

/**
 * Everything inside {@link FORK_FIXES_ROOT_ID}'s hydration boundary: the
 * hero header and the fixes list. The exact same function runs on both
 * sides of hydration: `fork-fixes-page.tsx`'s `ForkFixesPage` renders it
 * server-side (nested inside the {@link FORK_FIXES_ROOT_ID} container,
 * itself inside a full document), and `demo/fork-fixes-client.tsx` passes
      ...
      <header className="section-px" style={{ padding: '88px 80px 72px' }}>
        ...
        >
          ...
          >
            ...
            before the fix landed — the tree at that commit's parent — so
            nothing here is hand-drawn or reconstructed. The generator fails the
            build if any pair renders identically, because a before/after that
            looks the same would claim a fix it does not demonstrate.
          </p>
