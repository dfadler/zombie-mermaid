 ...
 * text getting cut off mid-word (#1031). `use-scroll-fade.ts`'s
 * `useScrollFade`/`scrollFadeMask` add the same edge-fade affordance
 * `diagram-detail-app.tsx`'s ASCII output panel already uses for the
 * identical problem — both extracted (#1051) from what used to be two
 * independent copies of this exact hook/mask pair.
 */
  ...
  return (
    <div
      ...
      style={{
        ...
        overflowX: 'auto',
        // Edge scroll-fade -- see scrollFadeMask's doc comment for why this
        // is a mask, not a painted overlay div. `undefined` (both false)
        // clears any mask, same as the property being unset.
        WebkitMaskImage: scrollFadeMask(fade.left, fade.right),
        maskImage: scrollFadeMask(fade.left, fade.right),
      }}
    >
      <div
        style={{
