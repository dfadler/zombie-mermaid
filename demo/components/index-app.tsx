 ...
 * latest-post value is a fixed marketing constant, not per-render data
 * (contrast `DashboardApp`'s `viewModel` or `ForkFixesApp`'s `fixes`) — so
 * there is nothing to serialize into a `<script type="application/json">`
 * props element, and `demo/index-client.tsx` calls `hydrateRoot()` with no
 * props to read back. The page's one piece of real per-build data, the
 ...
 * 2,200-line file):
 *
 * - `hero-visual.tsx` — {@link HeroVisual}
...
import { HeroVisual } from './hero-visual.tsx'
