...
import { scrollFadeMask, useScrollFade } from './use-scroll-fade.ts'
...
export function DetailOutputPanel({
  ...
  // Drives the ASCII edge fade -- see use-scroll-fade.ts's scrollFadeMask/
  // useScrollFade.
  const asciiFade = useScrollFade(
    renderAreaRef,
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          // Edge scroll-fade for the ASCII state -- see use-scroll-fade.ts's
          // scrollFadeMask doc comment for why this is a mask, not a painted
          // overlay div. `undefined` (both false, or outside ASCII mode/
          // fullscreen -- see useScrollFade's `enabled` gate) clears any
          // mask, same as the property being unset.
          WebkitMaskImage: scrollFadeMask(asciiFade.left, asciiFade.right),
          maskImage: scrollFadeMask(asciiFade.left, asciiFade.right),
        }}
      >
        {/*
        ...
        >
          ...
          >
            ...
            <Card
              ...
              // tablet breakpoint override can win via plain cascade
              // order instead of needing !important to beat an inline
              // style (zombie-mermaid#1080). The separate
              // `.output-card:fullscreen` rule below that stays
              // deliberately unaffected — its own `flex: none` remains
              // `!important` rather than relying on that rule's already-
              // higher specificity, since this element's `background`/
              // `border-color` genuinely are still inline (Card's
              // `tone="glow"`/`accent` props) and this pass doesn't
              // re-verify that block beyond what it already documents.
              style={{
                minWidth: 0,
                display: 'flex',
                flexDirection: 'column',
