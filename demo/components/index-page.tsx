...
import { Card, CTA, Pill, SectionEyebrow, primitivesCss } from './primitives.tsx'
import {
  DesignFontLinks,
  FONT_SIZE,
  FONT_WEIGHT,
...
const SHOWCASE_THEMES = [
  ...
  { key: 'nord', label: 'Nord', bg: '#2e3440', node: '#4c566a', edge: '#88c0d0' },
  {
...
function HeroVisual() {
  return (
    <svg viewBox="0 0 700 460" width="100%" height="auto" style={{ display: 'block' }}>
      <rect
        x="20"
        y="50"
        width="260"
      ...
      <text x="36" y="110" className="mono" fontSize="13" fill={colorVar('--text-faint')}>
        graph TD
      </text>
      <text x="36" y="136" className="mono" fontSize="13" fill={colorVar('--text-dim')}>
        {'  Start --> '}
        <tspan fill={colorVar('--violet')}>Deploy</tspan>
        {'{Deploy?}'}
      </text>
      <text x="36" y="162" className="mono" fontSize="13" fill={colorVar('--text-dim')}>
        {'  Deploy -->|'}
      ...
      <text x="36" y="188" className="mono" fontSize="13" fill={colorVar('--text-dim')}>
        {'  Deploy -->|'}
      ...
      <rect x="430" y="50" width="140" height="50" rx="25" fill={colorVar('--blue')} />
      <text x="500" y="80" textAnchor="middle" fontSize="14" fontWeight="700" fill="#081018">
        Start
      </text>
      ...
      <polygon points="500,140 565,175 500,210 435,175" fill={colorVar('--violet')} />
      <text x="500" y="180" textAnchor="middle" fontSize="13" fontWeight="700" fill={colorVar('--bg')}>
        Deploy?
      </text>
      ...
      />
      <text x="378" y="240" fontSize="12" fill={colorVar('--green')} className="mono">
        yes
      </text>
      ...
      />
      <text x="600" y="240" fontSize="12" fill={colorVar('--pink')} className="mono">
        no
      </text>

      <rect x="382" y="300" width="130" height="50" rx="12" fill={colorVar('--green')} />
      <text x="447" y="330" textAnchor="middle" fontSize="14" fontWeight="700" fill="#081018">
        Ship it
      </text>

      <rect x="546" y="300" width="130" height="50" rx="12" fill={colorVar('--pink')} />
      <text x="611" y="330" textAnchor="middle" fontSize="14" fontWeight="700" fill="#2a0a18">
        Iterate
      </text>
...
function Hero() {
  return (
    ...
    >
      <div
        className="hero-copy"
        style={{ flex: '0 0 500px', display: 'flex', flexDirection: 'column', gap: `${SPACE['3xl']}px` }}
      >
        <SectionEyebrow>An actively maintained fork of beautiful-mermaid</SectionEyebrow>
        <h1 className="hero-h1" style={{ fontSize: '56px', lineHeight: 1.08, letterSpacing: LETTER_SPACING.display }}>
          Your diagrams deserve more than one gray theme.
        </h1>
        <p style={{ fontSize: '19px', lineHeight: 1.6, color: colorVar('--text-dim'), maxWidth: '480px' }}>
        ...
        >
          <Pill mono style={{ background: colorVar('--panel'), border: `1px solid ${colorVar('--border')}`, color: colorVar('--text') }}>
      ...
      <div className="hero-visual" style={{ flex: '0 0 700px', maxWidth: '100%' }}>
        <HeroVisual />
      </div>
    </div>
  )
  ...
  return (
    <Card padding={20} style={{ background: theme.bg }}>
      <svg viewBox="0 0 160 90" width="100%" height="90">
        <rect x="10" y="15" width="60" height="34" rx="10" fill="none" stroke={theme.node} strokeWidth="2" />
        <rect x="90" y="42" width="60" height="34" rx="10" fill="none" stroke={theme.edge} strokeWidth="2" />
        <path d="M60 40 L100 55" stroke={theme.edge} strokeWidth="2" className="edge-anim" fill="none" />
      </svg>
      <p style={{ marginTop: `${SPACE.lg}px`, fontSize: `${FONT_SIZE.bodySm}px`, fontWeight: FONT_WEIGHT.bold, textAlign: 'center', color: theme.edge }}>
        {theme.label}
      </p>
    </Card>
  )
...
function ThemeShowcase() {
  return (
    ...
    >
      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: `0 auto ${SPACE['6xl']}px auto`, display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        ...
        <p style={{ fontSize: `${FONT_SIZE.lead}px`, color: colorVar('--text-dim'), maxWidth: `${LAYOUT.proseMaxWidth}px` }}>
          Themes are pure CSS custom properties, so switching one is instant. Here are five of the fifteen — the same node-and-edge shape wearing five different looks.
        </p>
      </div>
...
function FeatureGrid() {
  return (
    <div className="section-px" style={{ padding: `${SECTION_SPACE.hero}px ${LAYOUT.gutter.desktop}px ${SECTION_SPACE.loose}px ${LAYOUT.gutter.desktop}px` }}>
      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto 72px auto', display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        <SectionEyebrow>Built for how diagrams get used now</SectionEyebrow>
        <h2 style={{ fontSize: '38px', letterSpacing: LETTER_SPACING.heading }}>Six nodes, one rendering engine.</h2>
      </div>

      <div className="feature-grid-wrap" style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto', position: 'relative', height: '464px' }}>
        <svg
        ...
        >
          <path d="M197 100 L639 100" stroke={colorVar('--blue')} strokeWidth="2.5" className="edge-anim" fill="none" />
          <path d="M639 100 L1081 100" stroke={colorVar('--violet')} strokeWidth="2.5" className="edge-anim" fill="none" />
          <path d="M1081 100 L1081 364" stroke={colorVar('--cyan')} strokeWidth="2.5" className="edge-anim" fill="none" />
          <path d="M1081 364 L639 364" stroke={colorVar('--pink')} strokeWidth="2.5" className="edge-anim" fill="none" />
          <path d="M639 364 L197 364" stroke={colorVar('--amber')} strokeWidth="2.5" className="edge-anim" fill="none" />
        </svg>
        ...
        >
          {FEATURE_ICONS.map((feature, i) => {
            ...
            return (
              <Card key={feature.name} padding={28} style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE.lg}px` }}>
                <FeatureIcon size={28} />
                <h3 style={{ fontSize: '18px' }}>{feature.label}</h3>
                <p style={{ fontSize: `${FONT_SIZE.body}px`, color: colorVar('--text-dim'), lineHeight: 1.5 }}>
                  {FEATURE_COPY[i]}
                </p>
              </Card>
            )
...
function CliPanel() {
  return (
    <div style={{ flex: '1 1 0', display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
      <div
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: `${SPACE.xs}px`, marginBottom: `${SPACE.xl}px` }}>
          <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ff6767', display: 'inline-block' }} />
          <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ffc85c', display: 'inline-block' }} />
          <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#5ee08a', display: 'inline-block' }} />
          <span className="mono" style={{ fontSize: `${FONT_SIZE.caption}px`, color: colorVar('--text-faint'), marginLeft: `${SPACE.xs}px` }}>
            terminal
          </span>
        </div>
        <div className="mono" style={{ fontSize: '13.5px', lineHeight: 1.9 }}>
          <TermLine>
            ...
            <span style={{ color: colorVar('--text') }}>zombie-mermaid render</span>{' '}
          ...
          <div style={{ marginTop: `${SPACE.sm}px` }}>
            <span style={{ color: colorVar('--text-faint') }}>$</span>{' '}
            <span style={{ color: colorVar('--text') }}>zombie-mermaid mcp</span>
          </div>
          <div style={{ color: colorVar('--green') }}>✓ MCP server listening on stdio</div>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE.xxs}px` }}>
        <h3 style={{ fontSize: '19px' }}>CLI</h3>
        <p style={{ fontSize: `${FONT_SIZE.body}px`, color: colorVar('--text-dim'), lineHeight: 1.55 }}>
          Render SVG, ASCII, HTML, or PNG straight from a script or CI job — themes, direction overrides, and terminal hyperlinks all pass through as flags.{' '}
          <a href={`${FORK_URL}#cli`}>Full flag reference →</a>
        </p>
      </div>
    </div>
  )
...
function McpRow({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: `${SPACE.md}px` }}>
      <div
        style={{
          flexShrink: 0,
          width: '30px',
...
function McpPanel() {
  return (
    <div style={{ flex: '1 1 0', display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
      <Card accent="violet" padding={24} style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: `${SPACE.md}px` }}>
          <Pill
            mono
            fontSize={12.5}
            style={{ background: colorVar('--panel-2'), border: `1px solid ${colorVar('--border')}`, color: colorVar('--text-dim'), padding: '6px 14px' }}
          >
          ...
          <Pill
            ...
            mono
            fontSize={11}
            style={{ padding: '5px 12px', textTransform: 'uppercase', letterSpacing: '0.06em' }}
          >
            Experimental
          </Pill>
        </div>

        <McpRow avatar={<TerminalIcon size={16} />} bubbleBorder="var(--border)">
          <p style={{ fontSize: `${FONT_SIZE.label}px`, color: colorVar('--text-faint') }}>Your coding agent calls a tool —</p>
          <p className="mono" style={{ fontSize: `${FONT_SIZE.label}px`, color: colorVar('--violet'), fontWeight: FONT_WEIGHT.bold }}>
        ...
        <McpRow avatar={<LogoMark size={16} />} bubbleBorder="var(--green)">
          <div style={{ display: 'flex', alignItems: 'center', gap: `${SPACE.md}px` }}>
            <svg viewBox="0 0 60 40" width="52" height="36" style={{ flexShrink: 0 }}>
              <rect x="4" y="4" width="20" height="12" rx="4" fill="none" stroke={colorVar('--blue')} strokeWidth="2" />
              <path d="M14 16 V24 H46 V24" stroke={colorVar('--green')} strokeWidth="2" fill="none" className="edge-anim" />
              <rect x="36" y="24" width="20" height="12" rx="4" fill="none" stroke={colorVar('--green')} strokeWidth="2" />
            </svg>
            <p style={{ fontSize: '12.5px', color: colorVar('--text-dim') }}>…and gets back a rendered SVG, no browser involved.</p>
          </div>
        </McpRow>
      </Card>
      <div style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE.xxs}px` }}>
        ...
        <p style={{ fontSize: `${FONT_SIZE.body}px`, color: colorVar('--text-dim'), lineHeight: 1.55 }}>
          <span className="mono">render_mermaid_svg</span>, <span className="mono">render_mermaid_ascii</span>, and a
          sequence-activation checker, exposed over stdio — embed it, or run <span className="mono">zombie-mermaid mcp</span>{' '}
          directly. Shipped to gauge interest, not a finished implementation — the tool surface may still change.{' '}
  ...
  return (
    ...
    >
      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: `0 auto ${SPACE['6xl']}px auto`, display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        ...
        <h2 style={{ fontSize: '38px', letterSpacing: LETTER_SPACING.heading }}>A real CLI. A real MCP server.</h2>
        <p style={{ fontSize: `${FONT_SIZE.lead}px`, color: colorVar('--text-dim'), maxWidth: `${LAYOUT.proseMaxWidth}px` }}>
      ...
      <div className="cli-mcp-row" style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto', display: 'flex', alignItems: 'stretch', gap: `${SPACE['5xl']}px` }}>
        <CliPanel />
        <McpPanel />
      </div>
    </div>
...
function FlowchartTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <rect x="6" y="8" width="34" height="18" rx="4" fill="none" stroke={colorVar('--blue')} strokeWidth="2" />
      <rect x="60" y="44" width="34" height="18" rx="4" fill="none" stroke={colorVar('--blue')} strokeWidth="2" />
      <path d="M23 26 V44 H77 V44" stroke={colorVar('--blue')} strokeWidth="2" fill="none" className="edge-anim" />
    </svg>
  )
}
...
function StateTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <circle cx="24" cy="20" r="14" fill="none" stroke={colorVar('--violet')} strokeWidth="2" className="radar-ping" />
      <circle cx="24" cy="20" r="14" fill="none" stroke={colorVar('--violet')} strokeWidth="2" />
      <circle cx="76" cy="50" r="14" fill="none" stroke={colorVar('--violet')} strokeWidth="2" />
      <path d="M36 28 L64 42" stroke={colorVar('--violet')} strokeWidth="2" fill="none" />
    </svg>
  )
}
...
function SequenceTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <line x1="22" y1="8" x2="22" y2="62" stroke={colorVar('--cyan')} strokeWidth="2" />
      <line x1="78" y1="8" x2="78" y2="62" stroke={colorVar('--cyan')} strokeWidth="2" />
      <path d="M22 24 H78" stroke={colorVar('--cyan')} strokeWidth="2" className="msg-flow-right" />
      <path d="M78 44 H22" stroke={colorVar('--cyan')} strokeWidth="2" className="msg-flow-left" />
    </svg>
  )
}
...
function ClassTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <rect x="22" y="8" width="56" height="50" rx="3" fill="none" stroke={colorVar('--amber')} strokeWidth="2" />
      <line x1="22" y1="26" x2="78" y2="26" stroke={colorVar('--amber')} strokeWidth="2" className="draw-line" />
      <line x1="22" y1="42" x2="78" y2="42" stroke={colorVar('--amber')} strokeWidth="2" className="draw-line" style={{ animationDelay: '0.5s' }} />
    </svg>
  )
}
...
function ERTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <rect x="4" y="24" width="30" height="20" rx="3" fill="none" stroke={colorVar('--pink')} strokeWidth="2" />
      <rect x="66" y="24" width="30" height="20" rx="3" fill="none" stroke={colorVar('--pink')} strokeWidth="2" />
      <polygon points="50,20 60,34 50,48 40,34" fill="none" stroke={colorVar('--pink')} strokeWidth="2" className="relation-pulse" />
      <path d="M34 34 H40 M60 34 H66" stroke={colorVar('--pink')} strokeWidth="2" className="relation-pulse" />
    </svg>
  )
}
...
function XYChartTile() {
  return (
    <svg viewBox="0 0 100 70" width="100%" height="70">
      <line x1="10" y1="8" x2="10" y2="62" stroke={colorVar('--green')} strokeWidth="2" />
      <line x1="10" y1="62" x2="94" y2="62" stroke={colorVar('--green')} strokeWidth="2" />
      <rect x="20" y="40" width="10" height="22" fill={colorVar('--green')} className="bar-grow" />
      <rect x="38" y="28" width="10" height="34" fill={colorVar('--green')} className="bar-grow" style={{ animationDelay: '0.2s' }} />
      <rect x="56" y="16" width="10" height="46" fill={colorVar('--green')} className="bar-grow" style={{ animationDelay: '0.4s' }} />
      <path d="M20 44 L46 30 L82 14" stroke={colorVar('--green')} strokeWidth="2" fill="none" className="edge-anim" />
    </svg>
  )
}

const GALLERY_TILES = [FlowchartTile, StateTile, SequenceTile, ClassTile, ERTile, XYChartTile]
  ...
  return (
    ...
    >
      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: `0 auto ${SPACE['6xl']}px auto`, display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        ...
        <h2 style={{ fontSize: '38px', letterSpacing: LETTER_SPACING.heading }}>Every shape your system needs to explain itself.</h2>
      </div>

      <div
        className="gallery-grid"
        style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: `${SPACE['3xl']}px` }}
      >
        {GALLERY_TYPES.map((type, i) => {
          ...
          return (
            <Card
              ...
              padding={18}
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: `${SPACE.md}px` }}
            >
              <Tile />
              <p style={{ fontSize: '13.5px', fontWeight: FONT_WEIGHT.bold, textAlign: 'center', color: colorVar('--text') }}>{type.label}</p>
            </Card>
          )
        })}
      </div>

      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: `${SPACE['5xl']}px auto 0 auto`, textAlign: 'center' }}>
  ...
  return (
    <div className="stat-row" style={{ display: 'flex', justifyContent: 'space-between' }}>
      <div>
        ...
        <p style={{ fontSize: '13.5px', color: colorVar('--text-dim') }}>since last commit</p>
      </div>
      <div>
        <p className="display" style={{ fontSize: '36px', color: ink }}>
          {mergedPRs}
        </p>
        <p style={{ fontSize: '13.5px', color: colorVar('--text-dim') }}>merged PRs</p>
      </div>
      <div>
        <p className="display" style={{ fontSize: '36px', color: ink }}>
          {openPRs}
        </p>
        <p style={{ fontSize: '13.5px', color: colorVar('--text-dim') }}>open PRs</p>
      </div>
    </div>
  )
}
...
function ProofSection() {
  return (
    <div id="fixes" className="section-px" style={{ padding: `${SECTION_SPACE.hero}px ${LAYOUT.gutter.desktop}px` }}>
      <div style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: `0 auto ${SPACE['6xl']}px auto`, display: 'flex', flexDirection: 'column', gap: `${SPACE.xl}px` }}>
        <SectionEyebrow>Straight from the live maintenance dashboard</SectionEyebrow>
        <h2 style={{ fontSize: '38px', letterSpacing: LETTER_SPACING.heading }}>Actively maintained. Not abandoned.</h2>
        <p style={{ fontSize: `${FONT_SIZE.lead}px`, color: colorVar('--text-dim'), maxWidth: `${LAYOUT.proseMaxWidth}px` }}>
          beautiful-mermaid stalled — dozens of open PRs, nothing merged in months. Here's the same fork, measured against the original as of {PROOF_SNAPSHOT.asOf} — see the{' '}
          <a href="dashboard.html">live dashboard</a> for current numbers.
        </p>
      </div>

      <div className="proof-grid" style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: `${SPACE['7xl']}px` }}>
        <Card accent="green" padding={36} style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE['4xl']}px` }}>
          <p style={{ fontSize: '15px', fontWeight: FONT_WEIGHT.bold, color: colorVar('--green'), letterSpacing: '0.02em' }}>
            zombie-mermaid (this fork)
          </p>
          <StatRow ink="var(--green)" {...PROOF_SNAPSHOT.fork} />
        </Card>

        <Card padding={36} style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE['4xl']}px`, opacity: 0.75 }}>
          <p style={{ fontSize: '15px', fontWeight: FONT_WEIGHT.bold, color: colorVar('--text-faint'), letterSpacing: '0.02em' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: `${SPACE['2xl']}px` }}>
          <ChecklistIcon size={34} />
          <div>
            <h3 style={{ fontSize: '19px' }}>What this fork fixes</h3>
            <p style={{ fontSize: `${FONT_SIZE.body}px`, color: colorVar('--text-dim') }}>
              {PROOF_SNAPSHOT.rescuedFixCount} documented bugs, each shown before/after with the actual pre-fix and post-fix code.
            </p>
          </div>
        </div>
        <CTA href="fork-fixes.html" accent="amber" style={{ whiteSpace: 'nowrap' }}>
          See the evidence
        </CTA>
      </Card>
    </div>
...
function BlogTeaser() {
  return (
    <div id="blog" className="section-px" style={{ padding: `80px ${LAYOUT.gutter.desktop}px ${SECTION_SPACE.hero}px ${LAYOUT.gutter.desktop}px` }}>
      <Card
        className="blog-teaser-card"
        padding={40}
        style={{ maxWidth: `${LAYOUT.maxWidth}px`, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: `${SPACE['5xl']}px` }}
      >
        <div className="blog-teaser-inner" style={{ display: 'flex', alignItems: 'center', gap: `${SPACE['4xl']}px` }}>
          <Pill mono style={{ background: colorVar('--panel-2'), border: `1px solid ${colorVar('--border')}`, color: colorVar('--text-faint'), fontSize: `${FONT_SIZE.label}px` }}>
            {LATEST_POST.displayDate}
          </Pill>
          <div style={{ display: 'flex', flexDirection: 'column', gap: `${SPACE.xs}px`, maxWidth: '560px' }}>
            <h3 style={{ fontSize: '21px' }}>
              <a href={`blog/${LATEST_POST.slug}.html`} style={{ color: colorVar('--text') }}>
                {LATEST_POST.title}
              </a>
            </h3>
            <p style={{ fontSize: `${FONT_SIZE.body}px`, color: colorVar('--text-dim'), lineHeight: 1.5 }}>{LATEST_POST.description}</p>
          </div>
        </div>
        <a href="blog/" style={{ fontSize: `${FONT_SIZE.bodyLg}px`, fontWeight: FONT_WEIGHT.bold, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: `${SPACE.xxs}px` }}>
          Read the blog
...
const HOME_FOOTER_COLUMNS: readonly FooterColumn[] = [
  ...
  {
    title: 'Project',
    links: [{ label: 'MIT Licensed' }, { label: 'dfadler/zombie-mermaid', href: FORK_URL }],
  },
]
  ...
  return (
    <html lang="en">
      ...
      <body>
        ...
        <Nav hrefs={{ diagrams: 'diagrams/', editor: 'editor.html', forkFixes: 'fork-fixes.html', blog: 'blog/', github: FORK_URL }} />
        <main id="main">
          <Hero />
          <ThemeShowcase />
          <FeatureGrid />
