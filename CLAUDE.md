...
through `packages/site/ascii-html.ts`'s HTML approximation of a terminal, never a real
...
3. For a headless Chrome instance launched for screenshots, match on the `--headless`
   ...
   user's real browser.
4. Generated build artifacts (`index.html`, `editor.html`, `fork-fixes.html`,
   `blog/`) are gitignored — safe to leave, no cleanup needed. `public/hero.svg` is **not** gitignored
