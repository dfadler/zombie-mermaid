...
describe('Class Diagram Arrow Directions', () => {
  ...
  describe('Realization (..|>)', () => {
    test('implementation above interface - triangle points UP', () => {
      // Circle ..|> Shape means "Circle implements Shape".
      // Layout always places "from" above "to" regardless of relationship
      // type or which end the marker glyph is drawn at (matches ELK/
      // mermaid.com behavior — see class-diagram.ts's level-assignment
      // comment and issue #446), so Circle (from) is placed ABOVE
      // Shape (to/interface), with the hollow triangle pointing down
      // into Shape.
      const diagram = `classDiagram
        Circle ..|> Shape`
      const result = renderMermaidASCII(diagram)
    ...
    test('multiple implementations', () => {
      ...
      // Both implementations (from) above Shape (interface/to)
      const lines = result.split('\n')
      const shapeLine = lines.findIndex((l) => l.includes('Shape'))
      const circleLine = lines.findIndex((l) => l.includes('Circle'))
