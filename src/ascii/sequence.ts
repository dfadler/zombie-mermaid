...
export function renderSequenceAscii(
  ...
  for (let b = 0; b < diagram.blocks.length; b++) {
    ...
    // the block to fit it (#352). Measure the longest label among the
    // header and every divider up front and widen the wall to fit it
    // before any drawing happens. (Any participant this widening would
    // otherwise have swallowed was already pushed further right by the
    // lifeline-shifting layout pass above.)
    const hdrLabel = block.label ? `${block.type} [${block.label}]` : block.type
    ...
    if (bRight > blockCanvasMaxX) {
      // Grow to exactly bRight — not bRight + 1 — so a wall widened to fit
      // a label (#352) closes immediately after it with no trailing blank
      // column; #353's lifeline-clearance nudge above doesn't depend on
      // the extra column either, so this is safe for both.
